...
void main() {
  ...
  group('Streaming E2E - SSE Events Over Streamable HTTP', () {
    ...
    test('admin SSE stream receives event when agent registers', () async {
      // 1. Open admin SSE stream
      final sse = await _AdminSseClient.connect();

      // 2. Register agent via MCP tool call
      final regResult = await mcpClient.callTool('register', {
        'name': 'sse-agent-1',
      });
      ...
      expect(
        ...
        reason: 'MUST receive at least 1 SSE event after register',
      );
      ...
      expect(
        ...
        reason: 'SSE event MUST be an MCP logging notification',
      );
    });

    test('admin SSE stream receives events for ALL tool operations', () async {
      final sse = await _AdminSseClient.connect();
    ...
    test('SSE events contain correct payload structure', () async {
      final sse = await _AdminSseClient.connect();
      ...
      expect(eventJson['method'], equals('notifications/message'));
      ...
      expect(
        ...
        reason: 'Event type MUST be agent_registered for register',
      );
    });

    test('multiple SSE clients each receive all events', () async {
    ...
    test('SSE stream delivers events for concurrent tool calls', () async {
      final sse = await _AdminSseClient.connect();
      ...
      final regFutures = List.generate(
        agentCount,
        (i) => mcpClient.callTool('register', {'name': 'concurrent-$i'}),
      );
      await Future.wait(regFutures);

      // MUST receive events for all 5 registrations
      final events = await sse.waitForEvents(agentCount);
      sse.close();
    ...
    test('admin REST push delivers events to SSE stream', () async {
      ...
      expect(
        events.isNotEmpty,
        isTrue,
        reason: 'Admin REST push MUST deliver events to SSE',
      );
    });

    test('full round trip: register, lock, plan, message '
        'all stream as SSE events', () async {
      final sse = await _AdminSseClient.connect();
      ...
      final eventTypes = allEvents.map((e) {
        final json = jsonDecode(e) as Map<String, Object?>;
...
class _SseReader {
  ...
  static Future<_SseReader> open(
    ...
    // The entire async chain MUST be wrapped in
    // try-catch — when abort() is called, the
    // pending reader.read() rejects with AbortError
    // which would otherwise surface as an uncaught
    // async error and fail the test.
    unawaited(
      Future<void>(() async {
        try {
          ...
          // Stream aborted or errored — expected on
          // close()
        }
      }),
    );
...
Future<void> _adminPost(String path, Map<String, Object?> body) async {
  final headers = JSObject()..['Content-Type'] = 'application/json'.toJS;
  final options = JSObject()
    ..['method'] = 'POST'.toJS
    ..['headers'] = headers
