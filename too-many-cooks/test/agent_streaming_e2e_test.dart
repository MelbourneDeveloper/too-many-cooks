...
void main() {
  ...
  test(
    'agent receives message_sent notification via SSE',
    () async {
      await _resetServer();
  ...
  test(
    'agent receives lock_acquired notification via SSE',
    () async {
      await _resetServer();
      final agent1 = _McpClient();
      final agent2 = _McpClient();
  ...
  test(
    'agent receives agent_registered notification via SSE',
    () async {
      await _resetServer();
      final agent1 = _McpClient();
      final agent2 = _McpClient();
  ...
  test(
    'agent receives plan_updated notification via SSE',
    () async {
      await _resetServer();
      final agent1 = _McpClient();
      final agent2 = _McpClient();
  ...
  test(
    'agent receives lock_released notification via SSE',
    () async {
      await _resetServer();
      final agent1 = _McpClient();
      final agent2 = _McpClient();
  ...
  test(
    'agent notification has correct JSON-RPC payload structure',
    () async {
      await _resetServer();
      final agent1 = _McpClient();
      final agent2 = _McpClient();
...
class _AgentSseClient {
  ...
  static Future<_AgentSseClient> connect(
    String sessionId,
  ) async {
